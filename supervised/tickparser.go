...
// MAJORITY OF CODE DONE BY 87andrewh, found at: https://github.com/87andrewh/DeepAimDetector/blob/master/parser/to_csv.go
// MAJORITY OF CODE DONE BY 87andrewh, found at: https://github.com/87andrewh/DeepAimDetector/blob/master/parser/to_csv.go
// MAJORITY OF CODE DONE BY 87andrewh, found at: https://github.com/87andrewh/DeepAimDetector/blob/master/parser/to_csv.go
// MAJORITY OF CODE DONE BY 87andrewh, found at: https://github.com/87andrewh/DeepAimDetector/blob/master/parser/to_csv.go
...
func attackToString(data AttackData) []string {
	...
	for i := 0; i < samplesPerAttack; i++ {
		...
		out = append(out, fmt.Sprintf("%.3f", data.flashleft[i]))

		out = append(out, fmt.Sprintf("%.3f", data.velo[i].X))
		out = append(out, fmt.Sprintf("%.3f", data.velo[i].Y))
		out = append(out, fmt.Sprintf("%.3f", data.velo[i].Z))

		out = append(out, fmt.Sprintf("%.3f", data.attackerX[i]))
		out = append(out, fmt.Sprintf("%.3f", data.attackerY[i]))
		out = append(out, fmt.Sprintf("%.3f", data.attackerZ[i]))
		out = append(out, fmt.Sprintf("%.3f", data.victimX[i]))
		out = append(out, fmt.Sprintf("%.3f", data.victimY[i]))
		out = append(out, fmt.Sprintf("%.3f", data.victimZ[i]))
