...
class Killparser:
    ...
    def split_df_by_kill(self,mainfile,killfile,steamid,out_folder,write_to_csv=False):
        ...
        n_files = len([name for name in os.listdir(r'D:/Users/emill/csgocheaters/cleankills/a/')])
        cnt = 0
        players_this_game = self.get_players_ids()
        just_ids = [x[1] for x in players_this_game]
        print(just_ids)
        for p in just_ids:
            suspect = int(p)
...
def main(json_name, steamid):
    ...
    datamover.read_json(f"C:/Users/emill/PycharmProjects/csgoparse/128t-clean/a/{json_name}")
    ...
    out_folder = r"D:/Users/emill/csgocheaters/cleankills/a/"
...
import os
for cnt,filename in enumerate(os.listdir(r'C:/Users/emill/PycharmProjects/csgoparse/128t-clean/a')):
    with open('parsed_names.csv','a',newline='\n')as f:
        thewriter = csv.writer(f)
        thewriter.writerow([filename])
    if filename.endswith(".json"):
        print(filename,round(cnt/len(os.listdir(r'C:/Users/emill/PycharmProjects/csgoparse/128t-clean/a')),0))
        players_this_game = get_names(filename)
        just_name = [x[0] for x in players_this_game]

        json_name = filename
        steamid = [x[1] for x in players_this_game][0]
